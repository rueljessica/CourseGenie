...
export default class RecomendacaoGradesController {
    ...
    private async verificaEixos(disciplinasCursadas) {
        ...
        for (const disciplinaCursada of disciplinasCursadas) {
            ...
            if (disciplinaCursada.equivalenciaId && disciplina) {
                ...
                if (disciplinaEquivalente && disciplinaEquivalente.eixo && disciplinaEquivalente.eixo.length > 0) {
    ...
    private async discCursada(turma, disciplinasCursadas): Promise<boolean> {
        ...
        if (disciplina) {
            return disciplinasCursadas.some(disciplinaC =>
    ...
    private async atendePreRequisitos(turma, disciplinasCursadas): Promise<boolean> {
        ...
        if (disciplina) {
            ...
            return disciplina.preRequisitos.every(preRequisito =>
                disciplinasCursadas.some(disciplinaC =>
                    disciplinaC.codigo === preRequisito.codigo || disciplinaC.equivalenciaId === preRequisito.id
                )
            );
        }
        return false;
    ...
    private async obAnterioresPreReq(turma, periodo): Promise<boolean> {
        ...
        const validacao = await Promise.all(preRequisitos.map(async preRequisito => {
        ...
        if (turma.professorId) {
            ...
            if (disciplinasProfessor.length !== 0) {
                ...
                const totalAprovacoes = disciplinasFiltradas.filter((disciplinaCursada) =>
                    ['CUMPRIU', 'APR', 'APRN', 'INCORP', 'CUMP'].includes(disciplinaCursada.situacao.toUpperCase())
                ).length;
    ...
    private async processTurmas(turmas, disciplinasCursadas, periodoAluno, eixosCount, userId, limiteTurno) {
        let turmasDisponiveis = await Promise.all(turmas.map(async (turma) => {
            ...
            if (!atendePreRequisitos)
                return null;

            // Verifica se a turma atende aos requisitos de horario do aluno
            const contemLimiteTurno = limiteTurno.some(turno => turma.horario.toString().includes(turno));

            if (contemLimiteTurno)
                return null;
    ...
    private calculaPesos(turmasDisponiveis) {
        ...
        const maxPesoTotalSemPeriodoAtual = Math.max(
            ...turmasDisponiveis.filter(turma => turma.periodoAtual === 0).map(turma => turma.pesoTotal || 0)
        );
        ...
        turmasDisponiveis = turmasDisponiveis.map(turma => {
            if (turma.periodoAtual > 0) {
                ...
                return { ...turma, pesoTotal: parseFloat((turma.pesoTotal * ajusteProporcional).toFixed(1)) };
            } else {
                return turma;
            }
        });
    ...
    public async index({ response }: HttpContextContract) {
        ...
        const user = await User.findOrFail('16')
        ...
        const turmas = await Turma.query().where('ano', anoAtual).where('periodo', periodoAtual);
